...
import com.asoft.artsal.photo.business.collage.utils.GeometryUtils
...
class FrameImageView(
    ...
    private val autoLoadImage: Boolean = true
    ...
    private val mGestureDetector: GestureDetector = GestureDetector(
        ...
        object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                mOnImageClickListener?.onClickImage(this@FrameImageView)
                return true
            }
        }
    )
    var mTouchHandler: MultiTouchHandler? = null
    var image: Bitmap? = null
    private val mPaint: Paint = Paint()
    val mImageMatrix: Matrix = Matrix()
    ...
    interface OnImageClickListener {
        ...
        fun onClickImage(view: FrameImageView)

    }
    private var viewState: Bundle = Bundle.EMPTY

    init {
        // Chỉ load ảnh nếu autoLoadImage = true
        if (autoLoadImage) {
    ...
    suspend fun loadImageAsync() {
        try {
            if (photoItem.imagePath != null && photoItem.imagePath!!.toString().isNotEmpty()) {
                val bitmap = ImageDecoder.decodeFileToBitmap(context, photoItem.imagePath!!)
                image = bitmap
                resetImageMatrix()
                ...
                Timber.w("loadImageAsync - No image path available: path=${photoItem.imagePath}")
            }
        } catch (error: Exception) {
            Timber.e(error, "loadImageAsync - Failed to load image: ${photoItem.imagePath}")
    ...
    companion object {
        private fun setSpace(
            ...
            if (photoItem.path != null) {
                ...
                if (photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_3_3) {
                ...
                } else if (photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_USING_MAP && photoItem.shrinkMap != null) {
                    shrunkPoints = GeometryUtils.shrinkPathCollageUsingMap(
                        convertedPoints,
                        space,
                        photoItem.shrinkMap!!
                    )
                } else if (photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_COMMON && photoItem.shrinkMap != null) {
                    shrunkPoints =
        ...
        private fun buildRealPath(
            ...
            if (photoItem.path != null) {
                ...
                when (photoItem.cornerMethod) {
                    PhotoItem.Companion.CORNER_METHOD_3_6 -> {
                    ...
                    PhotoItem.Companion.CORNER_METHOD_3_13 -> {
                        outPath.computeBounds(bound, true)
                ...
                var x: Float
                var y: Float
                if (photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_3_6 || photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_3_8) {
        ...
        private fun buildRealClearPath(
            ...
            if (photoItem.clearPath != null) {
                ...
                var x: Float
                var y: Float
                if (photoItem.shrinkMethod == PhotoItem.Companion.SHRINK_METHOD_3_6) {
