...
import com.artsal.photo.editor.collage.maker.databinding.FragmentBeautifyBinding
...
class BeautifyFragment : BaseFragment<FragmentBeautifyBinding>() {
    ...
    override fun onInflateView(
        ...
        container: ViewGroup?
    ): FragmentBeautifyBinding {
        return FragmentBeautifyBinding.inflate(inflater, container, false)
    }
    ...
    override fun initListener() {
        binding?.apply {
            ...
            btnBeautify.setOnClickListener {
    ...
    private fun observeViewModel() {
        viewModel.isLoading.observe(viewLifecycleOwner) { isLoading ->
            ...
            binding?.btnBeautify?.isEnabled = !isLoading
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(androidx.lifecycle.Lifecycle.State.STARTED) {
                launch {
                    viewModel.beautifyResult.collect { result ->
                        result?.let { displayResult(it) }
                    }
                }
                launch {
                    viewModel.beautifyError.collect { error ->
                        error?.let { requireContext().toast("Beautify error: $it") }
                    }
                }
            }
        }
    ...
    private fun clearResults() {
        ...
        viewModel.clearBeautifyResult()
    }

    override fun renderUi() {
        // Initial UI state handled in setupUi
    }
}
